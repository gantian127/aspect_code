...
https://landlab.csdms.io/tutorials/marine_sediment_transport/simple_submarine_diffuser
_tutorial.html
...
if rank == 0:
    ...
    output_dir = os.path.join(
        os.getcwd(), f"perimeter_links_output_png_{num_partitions}"
    )
    os.makedirs(output_dir, exist_ok=True)

    output_pvtu = os.path.join(
        os.getcwd(), f"perimeter_links_output_pvtu_{num_partitions}"
    )
...
for time_step in time_steps:
    ...
    # cbar = fig.colorbar(sc, ax=ax)
    # cbar.set_label('Elevation (m)')
    # fig.savefig(os.path.join(output_sub_dir,
