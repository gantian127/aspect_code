...
Model to run on
simple elevation update function
...
for each_step in time_steps:
    run simple function to update elevation at one step
...
Notes:
    - this is based on original mpi_landlab_linear_diffusion.py
    - replace the linear diffusion model code at "run one ste"
    - this is aimed to test with large grid that the ghost node info exchange and
      local model's node and links are correctly structured to represent global grid.
...
if rank == 0:
    ...
    output_dir = os.path.join(
        os.getcwd(), f"simple_function_output_png_{num_partitions}"
    )
    os.makedirs(output_dir, exist_ok=True)

    output_pvtu = os.path.join(
        os.getcwd(), f"simple_function_output_pvtu_{num_partitions}"
    )
