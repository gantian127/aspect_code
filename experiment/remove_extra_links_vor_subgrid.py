...
for rank in range(0, num_partitions):
    ...
    if create_vor_grid:
        ...
        sc = ax.scatter(
            local_vmg.node_x,
            local_vmg.node_y,
            c=np.arange(len(local_vmg.node_x)),
            cmap="coolwarm",
            vmin=-3,
        )
            ...
            for link in links:
                if link != -1:
                    ...
                    if np.all(mask) and (link not in local_boundary_links):
                        ...
                        local_boundary_links_tail_head_nodes_global_ind.append(
                            tail_head_nodes
                        )
                        tail_head_nodes_local_ind = [
                            vmg_global_ind.index(node) for node in tail_head_nodes
                        ]
                        local_boundary_links_tail_head_nodes.append(
                            tail_head_nodes_local_ind
                        )
    ...
    local_vmg_adj = VoronoiDelaunayGrid(
        x.tolist(), y.tolist(), perimeter_links=local_boundary_links_tail_head_nodes
    )
    local_vmg_adj.status_at_node[local_boundary_nodes_ind] = (
        local_vmg_adj.BC_NODE_IS_FIXED_VALUE
    )
